...
Pressure relief valves (PRV, PSV, safety valve) are the last line of defense against overpressure in process equipment. They must be sized to pass the worst-case relieving scenario at an overpressure that does not exceed code-allowed accumulation (typically 10% above MAWP). Sizing follows API RP 520 (sizing equations) and API RP 521 (relief scenario identification). A relief device is one member of the wider valve family — compare duties and select across all valve types in the [Valve Types and Applications](/guides/valve-types-applications) guide, and understand the impact of line losses with the [Pressure Loss Calculator](/tools/pressure-loss-calculator).
