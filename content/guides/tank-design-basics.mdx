...
For sizing calculations on the vessels described above, use the [Tank Volume Calculator](/tools/tank-volume-calculator), [Tank Surface Area Calculator](/tools/tank-surface-area-calculator) and [Liquid Volume Calculator](/tools/liquid-volume-calculator). When the design pressure exceeds 15 psig the equipment must be treated as a pressure vessel — see the [Pressure Vessel Design](/guides/pressure-vessel-design) guide for ASME Section VIII rules and the [Pressure Vessel Volume Calculator](/tools/pressure-vessel-volume-calculator) for head and shell capacities. For shell and head weight estimates use the [Tank Weight Calculator](/tools/tank-weight-calculator); spherical geometry is handled by the [Sphere Volume Calculator](/tools/sphere-volume-calculator), and total stored capacity by the [Storage Capacity Calculator](/tools/storage-capacity-calculator).
